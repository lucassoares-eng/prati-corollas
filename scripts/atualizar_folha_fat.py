...
def get_folha_fat(diretoria, gerencia):
    ...
    tempBody = xl.parse(sheet_name= 'RELATÓRIO', skiprows=10, usecols=[3, 4], names= ['area', 'area_2'])
    for mes in range(meses):
        temp = {}
        dfBody = xl.parse(sheet_name= 'RELATÓRIO', skiprows=10, usecols=[4 + 8 * (mes + 1)])
        temp['mes'] = str(mes + 1)

    tempBody.loc[tempBody['area'].isnull(), 'area'] = tempBody['area_2']
